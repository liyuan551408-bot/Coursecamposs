...
function openCreate() {
  ...
  Object.assign(courseForm, emptyForm())
...
function openEdit(course) {
  ...
  Object.assign(courseForm, {
    ...
    description: course.description || '', workloadHours: course.workloadHours,
    level: course.level, offeredSemesters: [...(course.offeredSemesters || [])],
    assessmentTypes: [...(course.assessmentTypes || [])], officialLink: course.officialLink || '',
...
async function save() {
  ...
  try {
    ...
    ElMessage.error(err.response?.data?.message || 'Unable to save course.')
  ...
  <section class="admin-page">
    ...
    <div class="admin-grid">
      <el-card class="catalogue-card">
        <template #header><div class="card-header"><strong>Course catalogue</strong><el-input v-model="query" clearable placeholder="Search code or name" /></div></template>
        <el-table v-loading="loading" :data="filteredCourses" max-height="520">
          <el-table-column prop="code" label="Code" width="120" />
          <el-table-column prop="name" label="Course" min-width="220" />
          <el-table-column prop="level" label="Level" width="80" />
          <el-table-column prop="credits" label="Credits" width="85" />
          <el-table-column label="Status" width="90"><template #default="{ row }"><el-tag :type="row.isActive ? 'success' : 'info'">{{ row.isActive ? 'Active' : 'Inactive' }}</el-tag></template></el-table-column>
          <el-table-column label="" width="85" fixed="right"><template #default="{ row }"><el-button link type="primary" @click="openEdit(row)">Edit</el-button></template></el-table-column>
        </el-table>
      </el-card>
    ...
    <el-dialog v-model="dialogVisible" :title="editingId ? 'Edit course' : 'Create course'" width="min(720px, 94vw)" :close-on-click-modal="false">
      <el-form label-position="top">
        <div class="form-row"><el-form-item label="Course name"><el-input v-model="courseForm.name" /></el-form-item><el-form-item label="Course code"><el-input v-model="courseForm.code" placeholder="e.g. COMP101" /></el-form-item></div>
        ...
        <el-form-item label="Description"><el-input v-model="courseForm.description" type="textarea" :rows="3" /></el-form-item>
        <div class="form-row"><el-form-item label="Offered semesters"><el-select v-model="courseForm.offeredSemesters" multiple style="width:100%"><el-option label="Semester 1" value="SEMESTER_1" /><el-option label="Semester 2" value="SEMESTER_2" /><el-option label="Summer" value="SUMMER" /></el-select></el-form-item><el-form-item label="Assessment types"><el-select v-model="courseForm.assessmentTypes" multiple style="width:100%"><el-option v-for="item in ['EXAM','ASSIGNMENT','QUIZ','PROJECT','LAB','PRESENTATION']" :key="item" :label="item" :value="item" /></el-select></el-form-item></div>
        <el-form-item label="Prerequisites (IDs or exact course codes, comma-separated)"><el-input v-model="prerequisiteIdsInput" placeholder="e.g. 159.101, COMP102" /></el-form-item>
        <el-form-item label="Official course URL"><el-input v-model="courseForm.officialLink" type="url" /></el-form-item>
        <el-form-item v-if="editingId" label="Catalogue status"><el-switch v-model="courseForm.isActive" active-text="Active" inactive-text="Inactive" /></el-form-item>
      </el-form>
      <template #footer><el-button @click="dialogVisible = false">Cancel</el-button><el-button type="primary" :loading="saving" @click="save">{{ editingId ? 'Save changes' : 'Create course' }}</el-button></template>
    </el-dialog>
  </section>
</template>
