...
const route = useRoute(), router = useRouter(), authStore = useAuthStore(), savedStore = useSavedStore()
const loading = ref(true), submitting = ref(false), error = ref(''), course = ref(null), reviews = ref([])
=======
...
>>>>>>> 32e2bbe2b5ef1e578452c8017c33b44c0f0c2b8e
...
<<<<<<< HEAD
<template><section v-loading="loading" class="detail-page"><el-result v-if="error" icon="warning" title="Course unavailable" :sub-title="error"><template #extra><el-button @click="loadPage">Try again</el-button></template></el-result><template v-else-if="course"><el-button link @click="router.push('/courses')">← Back to course catalogue</el-button><div class="course-hero"><div><span class="course-code">{{ course.code }}</span><h1>{{ course.name }}</h1><p>{{ course.description || 'No course description is available.' }}</p></div><div class="rating-box"><b>{{ average }}</b><span>Course rating · {{ reviews.length }} reviews</span><el-button :type="savedStore.isSaved(course.id) ? 'warning' : 'primary'" plain size="small" class="save-btn" @click="savedStore.toggleSave(course.id); ElMessage.success(savedStore.isSaved(course.id) ? 'Added to saved courses' : 'Removed from saved courses')">{{ savedStore.isSaved(course.id) ? '★ Saved' : '☆ Save' }}</el-button></div></div><div class="detail-grid"><el-card><h2>Course information</h2><dl><dt>Credits</dt><dd>{{ course.credits }}</dd><dt>Estimated study time</dt><dd>{{ course.workloadHours || 'Not provided' }} {{ course.workloadHours ? 'hours' : '' }}</dd></dl></el-card><el-card><h2>Prerequisites</h2><p v-if="!course.prerequisites?.length" class="muted">This course has no prerequisites.</p><div v-else class="tag-list"><el-tag v-for="item in course.prerequisites" :key="item.id" @click="router.push(`/courses/${item.id}`)">{{ item.code }} · {{ item.name }}</el-tag></div><h3>Unlocks next</h3><p v-if="!course.prerequisiteFor?.length" class="muted">No follow-on courses have been recorded.</p><div v-else class="tag-list"><el-tag v-for="item in course.prerequisiteFor" :key="item.id" type="success" @click="router.push(`/courses/${item.id}`)">{{ item.code }}</el-tag></div></el-card></div><div class="review-layout"><div><h2>Student reviews</h2><el-empty v-if="!reviews.length" description="There are no approved reviews yet" /><article v-for="review in reviews" :key="review.id" class="review"><div><b>{{ review.user?.name || 'Anonymous student' }}</b><span class="muted"> · {{ review.user?.major || 'Student' }}</span></div><el-rate :model-value="review.overallRating" disabled /><p>{{ review.comment || 'This student did not leave a written review.' }}</p></article></div><el-card class="review-form"><h2>Write a review</h2><p class="muted">Your review will be published after approval.</p><el-form label-position="top"><el-form-item label="Overall recommendation"><el-rate v-model="form.overallRating" /></el-form-item><el-form-item label="Course difficulty"><el-rate v-model="form.difficultyRating" /></el-form-item><el-form-item label="Study workload"><el-rate v-model="form.workloadRating" /></el-form-item><el-form-item label="Your review"><el-input v-model="form.comment" type="textarea" :rows="4" maxlength="500" show-word-limit placeholder="Share your learning experience (optional)" /></el-form-item><el-button type="primary" :loading="submitting" @click="handleReview">Submit review</el-button></el-form></el-card></div></template></section></template>
<style scoped>.detail-page{max-width:1000px;margin:0 auto}.course-hero{display:flex;justify-content:space-between;gap:32px;padding:28px 0}.course-hero h1{margin:8px 0;font-size:40px}.course-hero p{max-width:670px;line-height:1.65}.course-code{color:var(--accent);font-weight:700}.rating-box{min-width:145px;text-align:center;border-left:1px solid var(--border);padding-left:24px}.rating-box b{display:block;font-size:38px;color:var(--text-h)}.rating-box span,.muted{color:var(--text);font-size:14px}.save-btn{margin-top:12px}.detail-grid,.review-layout{display:grid;grid-template-columns:repeat(2,1fr);gap:18px;margin:20px 0 40px}.detail-grid h2,.review-layout h2{margin-top:0}dl{display:grid;grid-template-columns:1fr 1fr;gap:12px}dt{color:var(--text)}dd{margin:0;font-weight:600}.tag-list{display:flex;flex-wrap:wrap;gap:8px}.tag-list .el-tag{cursor:pointer}h3{font-size:15px;margin:20px 0 8px}.review-layout{grid-template-columns:1.4fr .8fr}.review{padding:16px 0;border-bottom:1px solid var(--border)}.review p{margin-top:8px;line-height:1.55}.review-form{height:max-content}@media(max-width:700px){.course-hero,.detail-grid,.review-layout{grid-template-columns:1fr;display:grid}.rating-box{border-left:0;border-top:1px solid var(--border);padding:16px 0 0;text-align:left}.course-hero h1{font-size:32px}}</style>
=======
