<!-- @file Provides backend keyword, advanced, and semantic course search. -->
<script setup>
import { onBeforeUnmount, onMounted, reactive, ref, watch } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import { ElMessage } from 'element-plus'
import { searchCourses } from '../api/courses'
import { semanticSearchCourses } from '../api/ai'
...
const courses = ref([])
const searchMode = ref(route.query.mode === 'semantic' ? 'semantic' : 'keyword')
...
async function runSearch() {
  ...
  if (searchMode.value === 'semantic' && !query.value.trim()) {
    courses.value = []
    error.value = ''
    router.replace({ query: { mode: 'semantic' } })
    return
  }
  if (searchMode.value === 'semantic' && !authStore.isLoggedIn) {
    router.push({ name: 'Login', query: { redirect: `/courses?mode=semantic&q=${encodeURIComponent(query.value)}` } })
    return
  }
  ...
  try {
    ...
    courses.value = searchMode.value === 'semantic'
      ? await semanticSearchCourses(query.value.trim(), advanced)
      : await searchCourses({ keyword: query.value.trim(), ...advanced })
    router.replace({ query: { ...(query.value.trim() && { q: query.value.trim() }), ...(searchMode.value === 'semantic' && { mode: 'semantic' }) } })
  } catch (err) {
...
<template>
  <section class="course-page">
    <div class="page-heading">
      <div><p class="eyebrow">COURSE CATALOGUE</p><h1>Discover the right courses for you</h1><p>Search the complete catalogue with structured filters or meaning-based AI matching.</p></div>
    ...
    <el-card class="search-panel" shadow="never">
      <el-radio-group v-model="searchMode" class="mode-switch">
        <el-radio-button value="keyword">Keyword search</el-radio-button>
        <el-radio-button value="semantic">Semantic search</el-radio-button>
      </el-radio-group>
      <div class="search-row">
        <el-input v-model="query" clearable :placeholder="searchMode === 'semantic' ? 'Describe what you want to learn or achieve' : 'Search by course code, name or keyword'" size="large" @keyup.enter="runSearch"><template #prefix>⌕</template></el-input>
        <el-button size="large" type="primary" :loading="loading" @click="runSearch">Search</el-button>
        <el-button size="large" @click="showFilters = !showFilters">{{ showFilters ? 'Hide' : 'Advanced' }} filters</el-button>
      </div>
      <p v-if="searchMode === 'semantic'" class="mode-help">Semantic search matches intent, not just exact words. Sign-in is required because it uses the protected AI service.</p>
    ...
    <div v-loading="loading" class="course-grid">
      <el-card v-for="course in courses" :key="course.id" shadow="hover" class="course-card" @click="router.push(`/courses/${course.id}`)">
        ...
        <div class="tag-row"><el-tag v-if="course.level" size="small" effect="plain">Level {{ course.level }}</el-tag><el-tag v-for="semester in course.offeredSemesters || []" :key="semester" size="small" effect="plain">{{ semester.replace('_', ' ') }}</el-tag></div>
